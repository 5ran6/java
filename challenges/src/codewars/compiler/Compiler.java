...
/**
 *
 * brunolima on Oct 10, 2021
 * @see https://www.codewars.com/kata/5265b0885fda8eac5900093b/train/java
 */
...
public class Compiler {
	...
	public Ast pass1(String prog) {
		String functionArguments = extractArguments(prog);
		Map<String, Integer> functionArgumentsMap = calculateOrderArgumentsOnMap(functionArguments);

		String functionBody = extractArgumentsFromBody(prog);
	...
	String toPrefixNotation(String infixExpression) {
		...
		return prefixDeck.stream().collect(Collectors.joining(" "));
	}
	...
	private Ast optimize(Ast ast) {
		if (isBinop(ast)) {
			...
			if (isEligibleForContraction(a, b)) {
				...
				/**
				 * Optimize the case when + / \ * + /\ /\ 2 2 1 3
				 *
				 * became
				 *
				 * + / \ 4 4
	...
	private Map<String, Integer> calculateOrderArgumentsOnMap(String functionArguments) {
	...
	private String extractArguments(String params) {
	...
	private String extractArgumentsFromBody(String function) {
		Pattern pattern = Pattern.compile(FUNCTION_GROUPS_PATTERN);
	...
	/**
	 * Returns 0 if op0 precedence == op1 precedence Returns 1 if op0 has higher
	 * precedence Return -1 if op1 has higher precedence
	 */
	private static int firstHasHigherPrecedence(String op0, String op1) {
		if (("+".equals(op0) || "-".equals(op0)) && ("+".equals(op1) || "-".equals(op1))
				|| ("*".equals(op0) || "/".equals(op0)) && ("*".equals(op1) || "/".equals(op1))) {
			return 0;
